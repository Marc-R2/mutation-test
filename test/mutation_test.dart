...
void main() {
  group('mutation test', () {
    ...
    final exclusion = RegexRange(RegExp('/[*].*?[*]/', dotAll: true));

    test('Mutation Iteration with literal replacement in exclusion', () {
      var index = 0;
      const text2 = 'moo /* aaa */ xxx /* aaa */';
    ...
    test('Mutation with whitelist', () {
      final whitelist = LineRange(3, 10);
      var index = 0;
      const text2 = 'aaa \n aaa \n aaa \n aaa \n';
      final expected = [
        'aaa \n aaa \n bbb \n aaa \n',
      ...
      for (final modified in mut.allMutations(text2, [whitelist], [])) {
        ...
        index += 1;
        if (index == 1) {
          expect(modified.line.toMarkdown(), '''
Line 3:<br>
...
''');
    ...
    group('regex', () {
      ...
      final mutation2 = Mutation(RegExp('([a]+) ([b]+) ([c]+) ([d]+)'));

      final reg2 = RegexReplacement(r'($4 $3 $2 $1)');
